...
import { updateEmployeeProfile, getEmployeeProfile } from "@/lib/api";
...
export default function EmployeeProfilePage() {
  ...
  const [form, setForm] = useState({ name: "", phone: "", department: "", designation: "", password: "" });
  const [loading, setLoading] = useState(false);
  const [success, setSuccess] = useState("");

  useEffect(() => {
    getEmployeeProfile().then((r) => {
      const u = r.data;
      setUser(u);
      setForm({ name: u.name || "", phone: u.phone || "", department: u.department || "", designation: u.designation || "", password: "" });
    });
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const payload: any = { name: form.name, phone: form.phone, department: form.department, designation: form.designation };
      if (form.password) payload.password = form.password;
      const r = await updateEmployeeProfile(payload);
      setUser(r.data);
      ...
      <div style={{ maxWidth: "480px" }}>
        <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: "16px", padding: "28px", marginBottom: "16px", display: "flex", alignItems: "center", gap: "20px" }}>
          ...
          <div>
            ...
            <span style={{ fontSize: "11px", padding: "2px 10px", borderRadius: "20px", background: "rgba(99,102,241,0.2)", color: "#a5b4fc", fontWeight: 600 }}>EMPLOYEE</span>
            <p style={{ color: "rgba(255,255,255,0.35)", fontSize: "12px", margin: "6px 0 0" }}>{user?.email}</p>
            {user?.employeeId && <p style={{ color: "rgba(255,255,255,0.25)", fontSize: "11px", margin: "2px 0 0" }}>{user.employeeId}</p>}
          </div>
        </div>
