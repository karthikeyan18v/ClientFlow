...
import { getEmployeeProjects, updateProjectStatus } from "@/lib/api";
...
export default function EmployeeProjectsPage() {
  ...
  const load = () => getEmployeeProjects().then((r) => setProjects(r.data));
  useEffect(() => { load(); }, []);
    ...
    <div style={{ color: "white" }}>
      ...
      {projects.length === 0 && (
        <div style={{ textAlign: "center", padding: "60px", background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: "16px" }}>
          ...
          <p style={{ color: "rgba(255,255,255,0.3)", fontSize: "14px", margin: 0 }}>No projects assigned yet</p>
        </div>
      )}
                  ...
                  {colProjects.map((p) => (
                    <div key={p._id} style={{ background: "#13132a", border: "1px solid rgba(255,255,255,0.07)", borderRadius: "10px", padding: "12px" }}
                      ...
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "8px", marginTop: "8px" }}>
                        <span style={{ fontSize: "11px", color: "rgba(255,255,255,0.3)" }}>{p.clientId?.companyName || p.clientId?.name || "—"}</span>
                        <select value={p.status} onChange={(e) => handleStatus(p._id, e.target.value)}
                          style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "6px", color: "white", fontSize: "10px", padding: "3px 6px", cursor: "pointer", outline: "none" }}>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "13px" }}>
            <thead>
              <tr style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                {["Project", "Client", "Status", "Update", "Date"].map((h) => (
            ...
            <tbody>
              {projects.map((p) => (
                <tr key={p._id} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                  ...
                  <td style={{ padding: "14px 16px", color: "rgba(255,255,255,0.45)", fontSize: "12px" }}>{p.clientId?.companyName || p.clientId?.name || "—"}</td>
                  <td style={{ padding: "14px 16px" }}>
                  ...
                  <td style={{ padding: "14px 16px" }}>
                    <select value={p.status} onChange={(e) => handleStatus(p._id, e.target.value)}
                      style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "8px", color: "white", fontSize: "12px", padding: "6px 10px", cursor: "pointer", outline: "none" }}>
                      {COLUMNS.map((c) => <option key={c.key} value={c.key} style={{ background: "#13132a" }}>{c.label}</option>)}
                    </select>
                  </td>
